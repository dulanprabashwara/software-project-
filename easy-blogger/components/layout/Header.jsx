...
import { getSearchSuggestions } from "../../lib/searchApi"; // ← NEW import
...
export default function Header({ onToggleSidebar }) {
  ...
  // ── NEW: Close suggestions dropdown on outside click ──────────────────────
  useEffect(() => {
  ...
  const fetchSuggestions = useCallback((value) => {
    ...
    debounceTimer.current = setTimeout(async () => {
      try {
        ...
        const hasResults =
          (data?.articles?.length || 0) + (data?.users?.length || 0) > 0;
        setShowSuggestions(hasResults);
      } catch {
        // Silently ignore suggestion errors — they are non-critical
        setSuggestions({ articles: [], users: [] });
        setShowSuggestions(false);
      }
    }, 300);
  ...
  // ── NEW: Submit search → navigate to /home?q=term ─────────────────────────
  ...
  const handleSuggestionClick = useCallback(
    (term) => {
      setSearchQuery(term);
      setShowSuggestions(false);
      router.push(`/home?q=${encodeURIComponent(term.trim())}`);
    },
    [router]
  );
  // ─────────────────────────────────────────────────────────────────────────
  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 h-16 bg-white border-b border-[#e5e7eb] px-6 flex items-center justify-between">
      ...
      {/* ── MODIFIED BLOCK: Search (md+) ──────────────────────────────────── */}
      {/* Only this <div> changed from the original. Everything above and below
          this block is identical to the original Header.jsx.                  */}
      <div
              ...
              <>
                ...
                {suggestions.articles.map((a) => (
                  <button
                    ...
                    onClick={() => handleSuggestionClick(a.title)}
              ...
              <>
                ...
                {suggestions.users.map((u) => (
                  <button
                    ...
                    onClick={() => handleSuggestionClick(u.displayName || u.username)}
