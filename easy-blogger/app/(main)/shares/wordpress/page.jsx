...
export default function WordpressShares() {
  ...
  const loadArticles = useCallback(async (pageNum = 1) => {
    try {
      ...
      const wpArticles = filterWordpressArticles(rawArticles);

      setArticles((prev) =>
        pageNum === 1 ? wpArticles : [...prev, ...wpArticles]
      );
  ...
  return (
    <section className="px-8 min-w-0 w-full py-4">
      <div className="w-full">
        {isLoading ? (
          <p className="text-sm text-brand-muted py-6">Loading WordPress shared articles...</p>
        ) : articles.length === 0 ? (
