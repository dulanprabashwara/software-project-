...
export default function AdminDashboard() {
  ...
  if (!stats) return <div className="p-8 text-brand-muted">Loading Dashboard Analytics...</div>;
  ...
  const chartOptions = {
    ...
    scales: {
      y: {
        min: 0,
        ticks: { stepSize: 100, color: '#6A7282' },
        grid: { color: '#E5E7EB', drawBorder: false }
      },
      x: {
        ticks: { color: '#6A7282', maxRotation: 45, minRotation: 45 },
    ...
    <div className="max-w-6xl mx-auto p-8 relative">
      ...
      <div className="grid grid-cols-4 gap-6 mb-10">
        <Link href="/admin/moderation/queue" className="block transform hover:scale-[1.02] transition-transform duration-200">
          <div className="bg-linear-to-br from-brand-primary to-brand-primary-hover rounded-2xl p-6 shadow-md text-center h-full flex flex-col justify-center">
        ...
        <button
          ...
          className={`block transform hover:scale-[1.02] transition-all text-left outline-none rounded-2xl ${view === "engagement" ? 'ring-4 ring-brand-blue ring-offset-2' : ''}`}
        >
          <div className={`${view === "engagement" ? 'bg-brand-blue' : 'bg-linear-to-br from-brand-primary to-brand-primary-hover'} rounded-2xl p-6 text-center h-full flex flex-col justify-center shadow-md transition-colors`}>
      ...
      <div className="bg-gray-200/60 p-4 rounded-[2.5rem]">
        <div className="bg-white rounded-3xl p-8 shadow-sm min-h-125 flex flex-col">
          <div className="flex justify-between items-center mb-8">
            <h2 className="text-xl font-bold text-brand-muted flex items-center gap-3">
              {view === "engagement" && (
            ...
            <div className="flex gap-4 items-center">
              {view === "platform" && (
                <button
                  onClick={downloadCSV}
                  className="flex items-center gap-2 bg-brand-blue hover:bg-brand-blue-hover text-white px-4 py-1.5 rounded-xl text-sm font-semibold transition-colors shadow-sm"
                >
                  <Download size={16} />
                  Export CSV
                </button>
              ...
              {view === "engagement" && (
                <div className="flex items-center gap-2 bg-gray-50 px-3 py-1.5 rounded-xl border border-gray-100">
                  ...
                  <select
                    ...
                    className="bg-transparent text-sm font-semibold text-brand-muted outline-none cursor-pointer"
                  >
          ...
          <div className="flex-1 w-full relative">
            {chartLoading ? (
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center bg-white/60 backdrop-blur-[2px] rounded-2xl transition-all">
                <Loader2 className="w-10 h-10 animate-spin text-brand-blue mb-3" />
                <p className="text-sm font-bold text-brand-muted animate-pulse">Syncing latest data...</p>
              </div>
            ) : null}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6 mt-8">
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6 relative overflow-hidden">
          ...
          {feedsLoading ? (
            ...
            <p className="text-sm text-brand-muted py-4">No recent articles found.</p>
          ) : (
              ...
              {feeds.recentArticles.map((article) => (
                <div key={article.id} className="flex items-center justify-between border-b border-gray-50 pb-4 last:border-0 last:pb-0">
                  <div className="flex items-center gap-3">
                    ...
                    />
                    <div>
                      <p className="text-sm font-semibold text-brand-black truncate max-w-[200px]">{article.title}</p>
                      <p className="text-xs text-brand-muted">{article.author?.displayName || 'Unknown Author'}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase tracking-wider ${article.status === 'PUBLISHED' ? 'bg-[#E8F8F5] text-brand-primary' : 'bg-gray-100 text-brand-muted'}`}>
                      {article.status}
                    </span>
        ...
        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 p-6">
          ...
          {feedsLoading ? (
            ...
            <p className="text-sm text-brand-muted py-4">No activity events found yet.</p>
          ) : (
              ...
              {feeds.recentActivity.map((event, index) => (
                <div key={event.id} className="flex gap-4">
                  ...
                  <div className="flex flex-col items-center relative min-w-[12px]">
                    <div className="w-3 h-3 rounded-full border-2 border-brand-blue bg-white mt-1.5 z-10 relative" />
