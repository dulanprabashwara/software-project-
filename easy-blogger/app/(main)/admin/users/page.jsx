...
export default function UserListPage() {
  ...
  const [activeFilters, setActiveFilters] = useState({
    regular: false, premium: false, banned: false, active: false,
  });
  ...
  const handleToggleClick = (user) => {
    if (user.status === "Active") {
      setBanningUser(user);
      setValidationError(""); // Reset validation when opening
    } else {
  ...
  const updateUserStatus = async (id, newStatus, reason) => {
    ...
    try {
      ...
      if (response.ok) {
        console.log("✅ PostgreSQL Audit Entry Created:", auditData);
  ...
  const filteredUsers = users.filter(user => {
    ...
    const matchesSearch = (user.name?.toLowerCase() || "").includes(cleanQuery) || 
  ...
  return (
    <div className="p-8 bg-white min-h-screen relative overflow-hidden">
      ...
      {/* Table Container - Blurs when Side Panel is active */}
              ...
              {loading ? (
                <tr>
                  <td colSpan="3" className="text-center p-20">
                    ...
                    <p className="text-xs text-gray-400 mt-2 font-bold italic uppercase tracking-widest">Fetching From Backend...</p>
                  </td>
                </tr>
              ) : (
        ...
        {selectedUser && (
          <div className="p-10 relative flex flex-col items-center h-full">
            ...
            <div className="bg-white p-10 rounded-[35px] w-full text-center shadow-sm">
              ...
              <div className="mt-8 text-[11px] text-gray-400 space-y-2 font-bold italic">
          ...
          <div className="bg-white p-8 rounded-[40px] shadow-2xl max-w-sm w-full border border-gray-100 animate-in zoom-in duration-200">
            ...
            <textarea 
               ...
               onChange={(e) => {
                 ...
                 if(e.target.value.trim().length >= 10) setValidationError(""); // Clear error while typing
               }}
            />
            {validationError && (
