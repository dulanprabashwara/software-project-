...
export default function AdminProfilePage() {
  ...
  if (loading) return <div className="p-8 text-brand-muted font-bold">Loading Data...</div>;
    ...
    <div className="flex gap-12 p-8 bg-white max-w-7xl mx-auto">
      <div className="flex-1 text-left">
        <div className="flex justify-between items-center mb-6">
          <h1 className="text-4xl font-bold text-brand-black" style={{ fontFamily: "Georgia, serif" }}>{adminData.name}</h1>
        </div>

        <div className="space-y-10 mt-8">
          <section>
            <p className="text-brand-muted leading-relaxed text-base mb-6">{adminData.bio}</p>
          ...
          <section className="pt-8 border-t border-gray-50">
            ...
            <div className="flex flex-wrap gap-2">
              {adminData.permissions.map((perm, i) => (
                <span key={i} className="px-3 py-1 bg-blue-50 text-brand-blue border border-blue-100 text-[10px] font-bold rounded-lg uppercase">{perm}</span>
              ))}
            </div>
          </section>
          ...
          <section className="pt-8 border-t border-gray-50">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="p-6 bg-[#F0FDFA] rounded-3xl border border-[#CCFBF1]">
                ...
                <h4 className="text-2xl font-black text-brand-black">{adminData.stats.actions}</h4>
                <p className="text-[10px] font-bold text-brand-muted uppercase">System Actions Logged</p>
              </div>
              <div className="p-6 bg-[#EFF6FF] rounded-3xl border border-[#DBEAFE]">
                <Shield className="text-brand-blue mb-4" size={24} />
                <h4 className="text-2xl font-black text-brand-black">{adminData.stats.resolved}</h4>
                <p className="text-[10px] font-bold text-brand-muted uppercase">Reports Resolved</p>
              </div>
            </div>
          </section>
            ...
            {adminData.sessions.map((session) => (
              <div key={session.id} className="flex items-center justify-between p-4 bg-gray-50 rounded-2xl border border-gray-100 mb-3">
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-sm font-bold text-brand-black">{session.device}</p>
      ...
      <div className="w-64 pt-4 sticky top-8 h-fit text-left">
        <div className="w-24 h-24 bg-gray-200 rounded-full mb-6 flex items-center justify-center overflow-hidden text-brand-muted text-3xl font-bold shadow-sm">
          {adminData.avatar ? <img src={adminData.avatar} alt="Profile" className="w-full h-full object-cover" /> : adminData.name.charAt(0)}
        </div>
        <h2 className="text-lg font-bold text-brand-black mb-1">{adminData.name}</h2>
        <div className="text-xs text-brand-muted space-y-1 mb-4">
          ...
          <p className="font-bold text-brand-primary uppercase text-[10px] tracking-widest mt-2 flex items-center gap-1"><Shield size={12} /> {adminData.role}</p>
        </div>
        <Link href="/admin/profile/edit" className="text-brand-primary text-sm font-medium hover:underline block mb-8 text-[13px]">Edit profile</Link>
      </div>
    </div>
  );
}