...
export default function AdminSupportTickets() {
  ...
  const getDropdownColor = () => {
    ...
    if (filterStatus === 'RESOLVED') return 'bg-brand-primary text-white';
  ...
  return (
    <div className="flex h-[calc(100vh-100px)] gap-6 p-6 bg-[#F9FAFB]">
      ...
      <div className="w-1/3 flex flex-col gap-4">
        <h1 className="text-2xl font-bold text-brand-black" style={{ fontFamily: "Georgia, serif" }}>Support Inbox</h1>
        ...
        <div className="flex gap-2 relative z-20">
          <div className="relative w-36" ref={dropdownRef}>
            ...
            {isDropdownOpen && (
              <div className="absolute top-full left-0 mt-1 w-full bg-white border border-gray-100 rounded-lg shadow-xl z-50 overflow-hidden">
                ...
                <button onClick={() => { setFilterStatus('RESOLVED'); setIsDropdownOpen(false); setPage(1); }} className="w-full text-left px-4 py-2 text-sm font-semibold text-brand-primary hover:bg-[#CCFBF1]">RESOLVED</button>
          ...
          <div className="relative flex-1 text-[#9CA3AF]">
            ...
            <input suppressHydrationWarning type="text" placeholder="Search by email..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-10 h-10 bg-white border border-[#E5E7EB] rounded-lg text-sm outline-none focus:border-brand-primary" />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto space-y-3 mt-2 pr-1 custom-scrollbar">
          {loading ? (
            ...
            filteredTickets.map((ticket) => (
              <div key={ticket.id} onClick={() => handleSelectTicket(ticket)} className={`p-4 rounded-xl border transition-all cursor-pointer ${selectedId === ticket.id ? "border-brand-primary bg-white shadow-md ring-1 ring-brand-primary" : "bg-white border-[#E5E7EB] hover:border-brand-primary"}`}>
                <div className="flex justify-between items-start mb-1">
                  <span className={`text-sm truncate pr-2 ${!ticket.isRead ? "font-bold text-brand-black" : "font-semibold text-brand-muted"}`}>
                    {ticket.email}
                  </span>
                  <div className={`w-2 h-2 rounded-full shrink-0 ${ticket.status === 'PENDING' ? 'bg-[#EAB308]' : 'bg-brand-primary'}`}></div>
                </div>
                <h3 className={`text-sm leading-tight line-clamp-2 ${!ticket.isRead ? 'font-semibold text-brand-black' : 'text-brand-muted'}`}>
                  {ticket.problem}
                </h3>
              </div>
            ))
          ...
          <>
            ...
            <div className="p-6 px-8 border-b border-gray-100 text-sm">
              <div className="space-y-0.5">
                <p className="text-brand-black"><span className="font-semibold">User Email:</span> <a href={`mailto:${activePost.email}`} className="text-brand-primary hover:underline">{activePost.email}</a></p>
                <p className="text-brand-black"><span className="font-semibold">Ticket ID:</span> {activePost.id}</p>
                <p className="text-brand-black"><span className="font-semibold">Time Submitted:</span> {new Date(activePost.createdAt).toLocaleString()}</p>
              </div>
            </div>

            {/* Body */}
            <div className="p-8 flex-1 overflow-y-auto pt-4">
              <h1 className="text-2xl font-bold text-brand-black mb-6 font-serif">Problem Description</h1>
              <p className="text-brand-muted leading-relaxed text-base whitespace-pre-wrap bg-gray-50 p-6 rounded-xl border border-gray-100">
                {activePost.problem}
              </p>
            </div>
            ...
            <div className="p-6 border-t border-[#E5E7EB] bg-[#F9FAFB] flex justify-end gap-4">
              {activePost.status === 'RESOLVED' ? (
                ...
                <button onClick={handleResolve} className="group flex items-center gap-3 bg-brand-primary hover:bg-brand-primary-hover text-white pl-6 pr-2 py-2 rounded-full active:scale-95 transition-all shadow-md cursor-pointer">
