  ...
  return (
    ...
    <div className="w-full bg-[#f0fdf9] p-4 rounded-4xl mb-10">
      ...
      <section>
          ...
          <div className="flex gap-5 overflow-x-auto bg-white p-3 rounded-2xl mb-3 ">
            {DATA.trending.map((article) => (
              <div key={article.id} className=" bg-white w-160 shrink-0 px-4 border-2 rounded-2xl border-[#e5e7eb]">
    ...
    <button
      key={topic.id}
      className="bg-white border-2 border-[#1abc9c] text-[#1abc9c] px-5 w-50 py-2 rounded-2xl text-[1.5rem]"
    >
      {topic.name}
    </button>
  ))}
...
<div className="text-right">
  <button className="bg-[#1abc9c] border-2 border-[white] text-white px-5  py-2 rounded-2xl text-[1rem]"> 
    More
  </button>
</div>
