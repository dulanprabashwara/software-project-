  ...
  return (
    <div className="min-h-screen bg-[#F8FAFC]">
      <div className="max-w-3xl mx-auto px-6 py-8">
        ...
        <p className="text-gray-500 mb-8">People you follow and their latest content</p>
        ...
        {activeTab === 'users' && (
          <div className="space-y-4">
            {followingUsers.map((user) => (
              <div key={user.id} className="bg-white p-4 rounded-xl shadow-sm flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 bg-gray-200 rounded-full"></div>
                  <div>
                    <p className="font-semibold text-[#111827]">{user.name}</p>
                    <p className="text-sm text-gray-500">{user.bio}</p>
                    <p className="text-xs text-gray-400 mt-1">{user.articlesCount} articles · {user.followersCount} followers</p>
                  </div>
                </div>
        ...
        {activeTab === 'articles' && (
          <div className="space-y-6">
            {latestArticles.map((article) => (
              <article key={article.id} className="bg-white p-6 rounded-xl shadow-sm">
